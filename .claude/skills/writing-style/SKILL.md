...
Most readers of this project are technical people who are not native English speakers. Many use a
translation tool. Write so that a reader or a tool cannot read a sentence in two ways.
...
2. Active voice: "The parser drops the item", not "The item is dropped".
...
- Use one word for one thing in a document. Do not change between "file", "input", and "source" for
  the same thing.
- Use common words. Define a domain term once, where the reader first sees it.
