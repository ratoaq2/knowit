...
Only `plans/` is local. When other people must see a decision, post it to the issue. Never commit large
or private sample files from `plans/`.

## Names

- Folder: `plans/<issue>-<slug>/`, for example `plans/42-short-slug/`. Without an issue:
  `plans/<yyyy-mm-dd>-<slug>/`.
- Branch: the same slug, see `CONTRIBUTING.md`. For example `fix/42-short-slug`.
